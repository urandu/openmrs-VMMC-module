//package org.openmrs.module.moh362.report.builder;
//
...
//import org.openmrs.module.moh362.report.VmmcReport.VmmcIndicatorLibrary;
//import org.openmrs.module.moh362.report.common.VmmcCommonDimensionLibrary;
...
//*/
//@Component
//@Builds({"moh362.common.report.indicator.moh362"})
//public class VmmcReport extends AbstractReportBuilder{
//    protected static final Log log = LogFactory.getLog(VmmcReport.class);
//
//    @Autowired
//    private VmmcCommonDimensionLibrary vmmcCommonDimensions;
//
//    @Autowired
...
//        cohortDsd.addDimension("age", ReportUtils.map(vmmcCommonDimensions.standardAgeGroups(), "onDate=${endDate}"));
//        cohortDsd.addDimension("gender", ReportUtils.map(vmmcCommonDimensions.gender()));
//
...
//        EmrReportingUtils.addRow(cohortDsd, "HV03", "Currently in care", ReportUtils.map(vmmcIndicators.currentlyInCare(), indParams), allColumns, Arrays.asList("14", "15", "16", "17", "18", "19"));
//
//        // 3.4 (Starting ART)
...
//
//        // 3.5 (Revisits ART)
//        EmrReportingUtils.addRow(cohortDsd, "HV03", "Revisits ART", ReportUtils.map(vmmcIndicators.revisitsArt(), indParams), allColumns, Arrays.asList("28", "29", "30", "31", "32", "33"));
//
//        // 3.6 (Currently on ART [All])
//        EmrReportingUtils.addRow(cohortDsd, "HV03", "Currently on ART [All]", ReportUtils.map(vmmcIndicators.currentlyOnArt(), indParams), allColumns, Arrays.asList("34", "35", "36", "37", "38", "39"));
//
//        // 3.7 (Cumulative Ever on ART)
//        EmrReportingUtils.addRow(cohortDsd, "HV03", "Cumulative ever on ART", ReportUtils.map(vmmcIndicators.cumulativeOnArt(), indParams), nonInfantColumns, Arrays.asList("40", "41", "42", "43", "44"));
//
//        // 3.8 (Survival and Retention on ART at 12 months)
//        cohortDsd.addColumn("HV03-45", "ART Net Cohort at 12 months", ReportUtils.map(vmmcIndicators.art12MonthNetCohort(), indParams), "");
//        cohortDsd.addColumn("HV03-46", "On original 1st Line at 12 months", ReportUtils.map(vmmcIndicators.onOriginalFirstLineAt12Months(), indParams), "");
//        cohortDsd.addColumn("HV03-47", "On alternative 1st Line at 12 months", ReportUtils.map(vmmcIndicators.onAlternateFirstLineAt12Months(), indParams), "");
//        cohortDsd.addColumn("HV03-48", "On 2nd Line (or higher) at 12 months ", ReportUtils.map(vmmcIndicators.onSecondLineAt12Months(), indParams), "");
//        cohortDsd.addColumn("HV03-49", "On therapy at 12 months (Total) ", ReportUtils.map(vmmcIndicators.onTherapyAt12Months(), indParams), "");
//
//        // 3.9 (Screening)
...
//        nonCohortDsd.addColumn("HV03-70", "HIV care visits (Females 18+)", ReportUtils.map(vmmcIndicators.hivCareVisitsFemale18(), indParams));
//        nonCohortDsd.addColumn("HV03-71", "HIV care visits (Scheduled)", ReportUtils.map(vmmcIndicators.hivCareVisitsScheduled(), indParams));
//        nonCohortDsd.addColumn("HV03-72", "HIV care visits (Unscheduled)", ReportUtils.map(vmmcIndicators.hivCareVisitsUnscheduled(), indParams));
//        nonCohortDsd.addColumn("HV03-73", "HIV care visits (Total)", ReportUtils.map(vmmcIndicators.hivCareVisitsTotal(), indParams));
//
//        return mergedDsd;
//    }
//}
